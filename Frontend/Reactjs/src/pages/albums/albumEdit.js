...
import { fetchPutDataWithAuth, fetchGetDataWithAuth } from 'client/client'; // Import hàm fetchPostDataWithAuth từ file client để gửi dữ liệu kèm theo xác thực
...
const EditAlbumForm = () => {
  ...
  useEffect(() => {
    ...
    if (!isLoggedIn) {
      // Nếu chưa đăng nhập (token không tồn tại)
  ...
  const [formData, setFormData] = useState({
    // Khởi tạo state formData để lưu dữ liệu từ form
  ...
  const [errors, setErrors] = useState({
    // Khởi tạo state errors để lưu các thông báo lỗi của form
  ...
  const handleInputChange = (e) => {
    ...
    setFormData((prevData) => ({
      // Cập nhật lại formData với giá trị mới của trường nhập liệu
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name.trim()) {
      // Nếu trường "name" trống hoặc chỉ chứa khoảng trắng
      newErrors.name = 'Name is required'; // Đặt lỗi cho trường "name"
      isValid = false; // Form không hợp lệ
    }

    if (!formData.description.trim()) {
      // Nếu trường "description" trống hoặc chỉ chứa khoảng trắng
    ...
    if (isValid) {
      const payload = {
        // Tạo payload chứa dữ liệu cần gửi lên server
      ...
      fetchPutDataWithAuth("/albums/"+album_id+"/update", payload) // Gọi API gửi dữ liệu lên server để thêm album
        .then((response) => {
  ...
  return (
    <form onSubmit={handleSubmit}>
      {' '}
      {/* Khi form được submit, gọi hàm handleSubmit */}
      <TextField
      ...
      <Button type="submit" variant="contained" color="primary">
        {' '}
        {/* Nút bấm submit form */}
        Edit Album {/* Nội dung hiển thị trên nút */}
      </Button>
    </form>
  );
};

export default EditAlbumForm; // Xuất component AddAlbumForm để sử dụng ở nơi khác
