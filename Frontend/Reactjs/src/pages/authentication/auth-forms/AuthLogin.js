...
const AuthLogin = () => {
  ...
  const handleLogin = async () => {
    ...
    fetchPostData('/auth/token', { email, password }) 
      .then((response) => {
        const { token } = response.data; // Lấy token từ response của server
        setLoginError(''); // Xóa lỗi đăng nhập nếu có
        localStorage.setItem('token', token); // Lưu token vào localStorage để ghi nhớ đăng nhập
  ...
  return (
    <Container component="main" maxWidth="xs"> 
      ...
      >
        Login {/* Nội dung hiển thị trên nút */}
      </Button>
...
export default AuthLogin; // Xuất component AuthLogin để có thể import và sử dụng ở nơi khác
